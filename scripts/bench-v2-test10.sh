...
# Mirrors Test 8 (kata-qemu) but for Cloud Hypervisor runtime.
# Validates K8s Pod Overhead mechanism for kata-clh RuntimeClass.
# ============================================================

RESULTS_DIR="/home/ec2-user/kata-benchmark-v2/results"
mkdir -p "$RESULTS_DIR"

CSV="$RESULTS_DIR/v2-test10-pod-overhead-clh.csv"
SUMMARY="$RESULTS_DIR/v2-test10-summary.md"
LOGFILE="$RESULTS_DIR/v2-test10-stdout.log"

NS="bench10"
TARGET_NODE="ip-172-31-18-5.us-west-2.compute.internal"
PAUSE_IMAGE="registry.k8s.io/pause:3.10"
HOSTPOD_NAME="bench10-hostpod"
RUNTIME_CLASS="kata-clh"

# Overhead values (based on Test 9: 167 MiB measured + 20% buffer ≈ 200Mi)
OVERHEAD_MEM="200Mi"
OVERHEAD_CPU="100m"
  ...
  template:
    ...
    spec:
      ...
      - name: stress
        ...
        args: ["--vm", "1", "--vm-bytes", "${stress_mem}M", "--vm-keep", "--timeout", "600s"]
        resources:
          requests:
...
scale_and_measure() {
  ...
  record_csv "10" "$phase" "$overhead_en" "$runtime" "$target" "$target" "$running" "$pending" "$failed" "$mem_avail" "$sched_mem" "$sched_cpu" "$oom"
...
main() {
  log "============================================================"
  log "=== Test 10: Pod Overhead Configuration Validation (kata-clh) ==="
  log "============================================================"
  ...
  record_csv "10" "0-pre" "false" "none" "0" "0" "0" "0" "0" "$mem_before" "$sched_mem_before" "$sched_cpu_before" "0"

  # Deploy 1 kata-clh pod without overhead
  log "Deploying 1 kata-clh pod (no overhead)..."
  gen_deployment_yaml "bench10-no-oh" "$RUNTIME_CLASS" 1 | kubectl apply -f - >/dev/null 2>&1
  scale_and_measure "bench10-no-oh" "$RUNTIME_CLASS" 1 "0-no-overhead" "false"

  # Check pod spec — should NOT have overhead field
  local pod_overhead
  pod_overhead=$(kubectl get pods -n "$NS" -l "app=bench10-no-oh" -o jsonpath='{.items[0].spec.overhead}' 2>/dev/null || echo "")
  ...
  kubectl delete deployment bench10-no-oh -n "$NS" --ignore-not-found >/dev/null 2>&1
  ...
  # Deploy 1 kata-clh pod with overhead
  log "Deploying 1 kata-clh pod (with overhead)..."
  gen_deployment_yaml "bench10-with-oh" "$RUNTIME_CLASS" 1 | kubectl apply -f - >/dev/null 2>&1
  scale_and_measure "bench10-with-oh" "$RUNTIME_CLASS" 1 "2-with-overhead" "true"

  # Verify overhead in pod spec
  pod_overhead=$(kubectl get pods -n "$NS" -l "app=bench10-with-oh" -o jsonpath='{.items[0].spec.overhead}' 2>/dev/null || echo "")
  ...
  if [[ -n "$pod_overhead" && "$pod_overhead" != "{}" && "$pod_overhead" != "null" ]]; then
    ...
    p2_overhead_mem=$(kubectl get pods -n "$NS" -l "app=bench10-with-oh" -o jsonpath='{.items[0].spec.overhead.memory}' 2>/dev/null || echo "")
    p2_overhead_cpu=$(kubectl get pods -n "$NS" -l "app=bench10-with-oh" -o jsonpath='{.items[0].spec.overhead.cpu}' 2>/dev/null || echo "")
    log "PASS: Pod overhead injected — memory=$p2_overhead_mem, cpu=$p2_overhead_cpu"
  else
    log "FAIL: Pod overhead NOT injected!"
  fi

  # Verify scheduler accounts for overhead
  ...
  log "Expected: mem >= $(( 128 + 200 ))Mi (container + overhead), cpu >= $(( 50 + 100 ))m"

  # Verify pod is running normally
  local p2_running
  p2_running=$(kubectl get pods -n "$NS" -l "app=bench10-with-oh" --field-selector=status.phase=Running --no-headers 2>/dev/null | wc -l)
  ...
  kubectl delete deployment bench10-with-oh -n "$NS" --ignore-not-found >/dev/null 2>&1
  sleep 30
  ...
  gen_deployment_yaml "bench10-scale-no-oh" "$RUNTIME_CLASS" 0 | kubectl apply -f - >/dev/null 2>&1

  for target in "${pod_steps[@]}"; do
    log ""
    log "--- Phase 3: Scaling to $target pods (no overhead) ---"
    scale_and_measure "bench10-scale-no-oh" "$RUNTIME_CLASS" "$target" "3-no-oh-$target" "false"
  ...
  # Cleanup
  kubectl delete deployment bench10-scale-no-oh -n "$NS" --ignore-not-found >/dev/null 2>&1
  log "Waiting for pod termination..."
  sleep 60
  ...
  gen_deployment_yaml "bench10-scale-oh" "$RUNTIME_CLASS" 0 | kubectl apply -f - >/dev/null 2>&1

  for target in "${pod_steps[@]}"; do
    log ""
    log "--- Phase 4: Scaling to $target pods (with overhead) ---"
    scale_and_measure "bench10-scale-oh" "$RUNTIME_CLASS" "$target" "4-oh-$target" "true"
  ...
  # Cleanup
  kubectl delete deployment bench10-scale-oh -n "$NS" --ignore-not-found >/dev/null 2>&1
  sleep 60

  # ========================================
  # Phase 5: Memory Pressure Validation (stress-ng, with overhead)
  # ========================================
  log ""
  ...
  gen_stress_deployment_yaml "bench10-stress" 0 "$stress_mem" | kubectl apply -f - >/dev/null 2>&1

  for target in "${stress_steps[@]}"; do
    log ""
    log "--- Phase 5: Scaling stress-ng pods to $target (${stress_mem}Mi each, with overhead) ---"

    kubectl scale deployment/bench10-stress -n "$NS" --replicas="$target" >/dev/null 2>&1
    log "Waiting up to 180s for pods..."
    kubectl wait -n "$NS" --for=condition=Ready pod -l "app=bench10-stress" --timeout=180s 2>/dev/null || true
    sleep 15

    local status_line
    status_line=$(collect_pod_status "bench10-stress")
    local running pending failed
    running=$(echo "$status_line" | awk '{print $1}')
    pending=$(echo "$status_line" | awk '{print $2}')
    failed=$(echo "$status_line" | awk '{print $3}')
    ...
    log "  target=$target running=$running pending=$pending failed=$failed MemAvail=${mem_avail}MiB sched_mem=${sched_mem}Mi sched_cpu=${sched_cpu}m OOM=$oom"
    record_csv "10" "5-stress-$target" "true" "$RUNTIME_CLASS" "$target" "$target" "$running" "$pending" "$failed" "$mem_avail" "$sched_mem" "$sched_cpu" "$oom"
  ...
  # Cleanup stress pods
  kubectl delete deployment bench10-stress -n "$NS" --ignore-not-found >/dev/null 2>&1
  sleep 60

  # ========================================
  # Phase 6: Finalize — keep overhead, cleanup
  # ========================================
  log ""
  ...
  {
    echo "# Test 10: Pod Overhead Configuration Validation (kata-clh)"
    echo ""
    echo "**Date:** $(date -u '+%Y-%m-%d %H:%M UTC')"
    echo "**Node:** $TARGET_NODE (r8i.2xlarge, 8 vCPU, 64GB RAM)"
    echo "**Allocatable:** 7910m CPU, 63084396Ki (~60GiB) memory"
    ...
    echo "**RuntimeClass:** kata-clh (Cloud Hypervisor)"
    echo ""
    echo "## Background"
    echo ""
    echo "Test 9 measured kata-clh (Cloud Hypervisor) VM memory overhead at ~167 MiB per pod."
    echo "This is lower than kata-qemu (~207 MiB) because Cloud Hypervisor is a lighter VMM."
    echo "With 20% safety buffer: 167 × 1.20 ≈ 200 MiB → overhead.podFixed.memory = 200Mi."
    echo ""
    echo "This test validates that K8s Pod Overhead works correctly with kata-clh, mirroring"
    echo "the Test 8 validation done for kata-qemu."
    echo ""
    ...
    if [[ -n "$p0_data" ]]; then
      ...
      echo "With 1 kata-clh pod (no overhead):"
    ...
    if [[ -n "$p2_data" ]]; then
      ...
      echo "With 1 kata-clh pod (with overhead):"
      echo "- Scheduler sees: mem=${p2_smem}Mi, cpu=${p2_scpu}m"
      echo "- Expected scheduler request per pod: $(( 128 + 200 ))Mi/${OVERHEAD_CPU}+${POD_CPU_REQ}=150m"
    ...
    echo "|--------|-------------|---------------|--------|"
    echo "| Per-pod scheduler mem request | ${POD_MEM_REQ} | $(( 128 + 200 ))Mi | +${OVERHEAD_MEM} |"
    ...
    echo "## Comparison with Test 8 (kata-qemu)"
    echo ""
    echo "| Parameter | kata-qemu (Test 8) | kata-clh (Test 10) |"
    echo "|-----------|--------------------|--------------------|"
    echo "| VMM | QEMU | Cloud Hypervisor |"
    echo "| Measured VM overhead | ~207 MiB/pod | ~167 MiB/pod |"
    echo "| Configured overhead | 250Mi | 200Mi |"
    echo "| Buffer percentage | ~20% | ~20% |"
    echo "| CPU overhead | 100m | 100m |"
    echo ""

    echo "## Key Findings"
    echo ""
    echo "1. **Pod Overhead injection works with kata-clh**: The admission controller correctly"
    echo "   injects \`.spec.overhead\` into pods using the kata-clh RuntimeClass."
    echo "2. **Scheduler accounts for overhead**: With overhead, the scheduler adds 200Mi memory"
    echo "   + 100m CPU to each pod's container requests."
    echo "3. **Lower overhead than kata-qemu**: Cloud Hypervisor has ~20% less VM overhead than"
    echo "   QEMU (167 vs 207 MiB), enabling higher pod density."
    echo "4. **Overhead prevents over-scheduling**: With accurate overhead accounting, the"
    echo "   scheduler prevents memory exhaustion by refusing to schedule beyond capacity."
    echo ""

    echo "## Final State"
    echo ""
    echo "RuntimeClass \`kata-clh\` now has Pod Overhead configured:"
    echo "\`\`\`yaml"
    echo "overhead:"
    echo "  podFixed:"
    ...
    echo "\`\`\`"
    echo ""
    echo "This is the recommended production configuration for kata-clh."
    echo ""
    echo "## CSV Data"
    echo ""
