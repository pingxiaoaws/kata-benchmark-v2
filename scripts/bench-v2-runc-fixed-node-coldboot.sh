...
# === runc Fixed-Node Cold Boot Benchmark ===
# Same node, same methodology as gVisor x86 test
# For apple-to-apple comparison of runtime startup overhead
...
CSV="$RESULTS_DIR/v2-test1-runc-fixed-node-boot-time.csv"
ITERATIONS=5
...
# ============================================================
log "=== runc Fixed-Node Cold Boot Benchmark ==="
...
for i in $(seq 1 $ITERATIONS); do
  kubectl delete namespace "bench-runc-${i}" --wait=false 2>/dev/null || true
done
sleep 5
...
for i in $(seq 1 $ITERATIONS); do
  local_ns="bench-runc-${i}"
  local_name="bench-runc-${i}"
  pod_name="${local_name}-0"
  ts=$(date -u '+%Y-%m-%dT%H:%M:%SZ')

  log "--- runc iteration $i / $ITERATIONS ---"
  ...
  if ! kubectl wait --for=condition=Ready "pod/$pod_name" -n "$local_ns" --timeout=300s >/dev/null 2>&1; then
    ...
    if ! kubectl wait --for=condition=Ready "pod/$pod_name" -n "$local_ns" --timeout=300s >/dev/null 2>&1; then
      ...
      echo "runc,$i,FAILED,$TARGET_NODE,$NODE_KERNEL,$NODE_ARCH,$NODE_INSTANCE,$ts" >> "$CSV"
      cleanup_ns "$local_ns"
      sleep 5
      continue
    fi
  ...
  echo "runc,$i,$boot_sec,$TARGET_NODE,$pod_kernel,$NODE_ARCH,$NODE_INSTANCE,$ts" >> "$CSV"
