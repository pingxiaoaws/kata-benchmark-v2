...
# kata-fc on c7g.metal (64 vCPU, 128 GiB, Graviton3 bare metal)
#
...
NODE="${1:-ip-172-31-16-251.us-west-2.compute.internal}"
RUNTIME="kata-fc"
MAX_PODS=70
SETTLE_SECS=60
RESULTS_CSV="results-fullload-c7g.csv"
LOG_FILE="test-fullload-c7g.log"

log() { echo "[$(date -u '+%Y-%m-%d %H:%M:%S UTC')] $*" | tee -a "$LOG_FILE"; }

: > "$LOG_FILE"
log "=== Max Pod Full-Load Test: $RUNTIME on c7g.metal ==="
log "64 vCPU, 128 GiB, Graviton3 bare metal, aarch64"
