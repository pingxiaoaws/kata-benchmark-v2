...
### ⚠️ 重要发现：gVisor 无法对 EFS 执行文件 I/O

测试日期：2026-04-16

使用 fio 对 EFS 挂载点进行读写性能测试时发现：

| 运行时 | EFS 挂载 | EFS 文件读写 | 错误信息 |
|--------|---------|-------------|---------|
| runc | ✅ | ✅ 正常 | — |
| gVisor | ✅ 挂载成功 | ❌ 失败 | `Operation not permitted` |

**gVisor sandbox 对 NFS 挂载点的文件写操作会被拦截**，即使 EFS PVC 成功挂载（mount 点存在且可见）。这意味着之前满载测试中的 `efs_mount_ok=OK` 仅验证了挂载点存在，但 **gVisor 中 EFS 实际不可用于数据读写**。

测试了两种 fio I/O 引擎（libaio 和 psync），以及直接 `dd` 命令，均报同样错误。

### runc EFS I/O 基线数据

fio 参数：iodepth=32, numjobs=4, runtime=60s, 文件大小=1G

| 测试项 | Block Size | 带宽 (MiB/s) | IOPS | 平均延迟 (μs) | P99 延迟 (μs) |
|--------|-----------|-------------|------|-------------|-------------|
| 顺序写 | 1M | 1,697.35 | 1,697 | 2,340.9 | 23,724.0 |
| 顺序读 | 1M | 3,649.82 | 3,650 | 1,074.9 | 20,840.4 |
| 随机写 | 4K | 249.61 | 63,902 | 49.9 | 10.5 |
| 随机读 | 4K | 10.81 | 2,768 | 1,444.1 | 4,489.2 |

> 注：EFS 的顺序读写带宽较高（得益于 NFS 客户端缓存），随机读性能受 NFS 延迟影响显著。

### 对生产环境的影响

- 如果工作负载需要在 gVisor sandbox 内读写 EFS 共享存储，**gVisor 不适用**
- runc 和 kata-qemu 均可正常使用 EFS
- gVisor + EFS 的组合仅适用于 **不需要持久化存储 I/O** 的纯计算型工作负载（如满载测试中的 stress-ng）
