...
NODE="${1:-ip-172-31-18-59.us-west-2.compute.internal}"
RUNTIME="kata-qemu"
...
# ---------- Cleanup function ----------
cleanup() {
  log "Cleaning up all maxpod-fl-* namespaces..."
  for ns in $(kubectl get ns --no-headers -o custom-columns=':metadata.name' | grep '^maxpod-fl-'); do
...
for N in $(seq 1 $MAX_PODS); do
  ...
  for i in $(seq 1 120); do
    STATUS=$(kubectl get pod "stress-${N}" -n "maxpod-fl-${N}" -o jsonpath='{.status.phase}' 2>/dev/null || echo "Unknown")
    READY_COND=$(kubectl get pod "stress-${N}" -n "maxpod-fl-${N}" -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}' 2>/dev/null || echo "False")
  ...
  TOTAL_RESTARTS=0
  for P in $(seq 1 $N); do
    POD_STATUS=$(kubectl get pod "stress-${P}" -n "maxpod-fl-${P}" -o jsonpath='{.status.phase}' 2>/dev/null || echo "Unknown")
    POD_RESTARTS=$(kubectl get pod "stress-${P}" -n "maxpod-fl-${P}" -o jsonpath='{.status.containerStatuses[*].restartCount}' 2>/dev/null || echo "0")
    R_SUM=0
  ...
  STRESS_CPU=$(kubectl top pod -n "maxpod-fl-${N}" --containers --no-headers 2>/dev/null | awk '{sum+=$3} END {gsub(/m/,"",sum); printf "%sm", sum}' || echo "N/A")
  STRESS_MEM=$(kubectl top pod -n "maxpod-fl-${N}" --containers --no-headers 2>/dev/null | awk '{sum+=$4} END {printf "%s", sum}' || echo "N/A")
