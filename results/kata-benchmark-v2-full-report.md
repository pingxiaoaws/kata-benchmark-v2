...
> gVisor 数据补充于 2026-04-21。gVisor 内核版本 4.4.0 是其用户态内核模拟版本号，非真实 Linux 内核。gVisor x86 节点为 m8i.2xlarge (8 vCPU, 32GB)，runc/kata 节点为 m8i.4xlarge (16 vCPU, 64GB)，实例规格差异对启动时间影响较小（启动主要受 I/O 和运行时初始化影响，非 CPU 密集型）。

### 结果分析

1. **首次冷启动**: kata-qemu 119.8s, kata-clh 107.1s — 包含 VM 镜像拉取和缓存建立；gVisor 78.6-78.9s — 包含镜像拉取和 runsc sandbox 初始化，无 VM 镜像开销
...
5. **gVisor 热启动最快**: x86 32.9s / arm64 33.2s，比 runc (51.8s) 快 **36%**，比 kata (70-72s) 快 **53%**。gVisor 没有 VM 启动开销，sandbox 创建仅为进程级操作（~200ms 运行时开销），热启动时间主要由应用本身的初始化决定
6. **gVisor x86 vs arm64 几乎无差异**: 冷启动 78.9s vs 78.6s，热启动 32.9s vs 33.2s，差距 <1%，说明 gVisor 的启动性能对 CPU 架构不敏感
...
|------|------|------|-----------------|
| 启动时间 (kata) | +18-20s (+37%) | ⚠️ 中等 | 可接受，通过预热缓解 |
| 启动时间 (gVisor) | -18s (-36%) † | ✅ 优秀 | 热启动比 runc 更快 |
| CPU 计算 | <1% | ✅ 优秀 | 无影响 |
| 内存带宽 | <5% | ✅ 优秀 | 无影响 |
...
| 文件 | 数据量 | 说明 |
|------|-------|------|
| `v2-test1-boot-time.csv` | 15 条 | 冷/热启动时间 (runc/kata-qemu/kata-clh) |
