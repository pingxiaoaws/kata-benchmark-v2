...
| gVisor † | 78.63s | 29.70s | 34.46s | 35.04s | 33.71s | 33.23s | 4.4.0 (gVisor) |

> † gVisor 数据补充于 2026-04-21。测试节点为 arm64 Graviton (m8gd.2xlarge, 8 vCPU, 32GB)，其余三种运行时的测试节点为 x86 (m8i.4xlarge, 16 vCPU, 64GB)。架构差异导致绝对时间不可直接横向对比，但冷/热启动 pattern 有参考价值。gVisor 内核版本 4.4.0 是其用户态内核模拟版本号，非真实 Linux 内核。

### 结果分析

1. **首次冷启动**: kata-qemu 119.8s, kata-clh 107.1s — 包含 VM 镜像拉取和缓存建立；gVisor 78.6s — 包含镜像拉取和 runsc sandbox 初始化，无 VM 镜像开销
...
5. **gVisor 热启动最快**: 33.2s，比 runc (51.8s) 快 36%，比 kata (70-72s) 快 53%。gVisor 没有 VM 启动开销，sandbox 创建仅为进程级操作（~200ms 运行时开销），热启动时间主要由应用本身的初始化决定
...
| `bench-v2-gvisor-coldboot.sh` | gVisor 冷启动测试 (Test 1 补充) |
| `v2-lib.sh` | 公共函数库 |
