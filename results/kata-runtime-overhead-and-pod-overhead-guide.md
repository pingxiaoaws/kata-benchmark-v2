...
> **📌 说明**: 初始测试（Test 5）发现 Kata VM 内的 `O_DIRECT` 标志被 virtiofs 缓存层拦截，数据实际写入 host 内存而非磁盘，导致测试结果不可比。**该项测试数据已移除**，后续将挂载 Amazon EBS（通过 CSI 驱动 + PVC）和 Amazon EFS 重新测试，以获得真实的持久化 I/O 性能对比数据。
...
kata-clh 比 kata-qemu 轻 18%，但每个 Pod 仍有 167 MiB 的内存消耗（cgroup 几乎不可见）。
...
|------|----------|---------------|--------------|
| `kubectl top pod` | 正常报告 | **严重低估**（~372 Ki vs 实际 ~275 MiB） | **严重低估**（同上） |
| Pod cgroup `memory.current` | 0.47 MiB | **~372 KiB**（实际 RSS ~275 MiB） | **~0 KiB**（实际 RSS ~150 MiB） |
| 调度器 request 计算 | 准确 | **严重低估** | **严重低估** |

**根因**: QEMU / cloud-hypervisor 进程在 pod cgroup 内，但 cgroup `memory.current` 严重低估其真实内存消耗。Guest RAM 通过 `/dev/shm` 以 `MAP_SHARED` 方式分配，cgroupv2 不将其计入；QEMU 二进制和共享库属于文件映射（page cache），同样不计入。结果是 ~275 MiB 的实际 RSS 仅被 cgroup 计入 <1 MiB。两种 VMM 都存在相同的 cgroup 低估问题，都需要 Pod Overhead。

#### 深入理解：Kata 的 Host Cgroup 双层架构

Kata Containers 运行在 **两层 cgroup** 之上（参考 [Kata Host Cgroup 设计文档](https://github.com/kata-containers/kata-containers/blob/main/docs/design/host-cgroups.md)）：

- **第一层（Guest 内）**: 容器工作负载在 VM 内的 cgroup，遵循标准 OCI 规范
- **第二层（Host 上）**: VMM（QEMU/CLH）及其关联线程（I/O 后端、vCPU 线程、Kata Shim）在 host 上的 cgroup

Kata 通过配置项 `sandbox_cgroup_only` 控制 host 层 cgroup 的行为，存在两种模式：

**模式 1: `sandbox_cgroup_only = true`（推荐，需配合 Pod Overhead）**

当配置了 Pod Overhead 后，Kubelet 会将 overhead 加入 Pod cgroup 的 size 计算中。此时 Kata 将 **所有进程**（VMM、vCPU 线程、I/O 线程、Kata Shim）放入 Pod cgroup 下的 `/kata_<PodSandboxID>` 子 cgroup：
...
✅ **优势**: 所有 Kata 进程在 Pod cgroup 内受限，cgroup 统计准确，防止 noisy neighbor。

**模式 2: `sandbox_cgroup_only = false`（默认值，不推荐生产使用）**

当**未配置** Pod Overhead 时，Pod cgroup 仅按容器 request 大小分配，不足以容纳 VMM 开销。Kata 会将 **非 vCPU 线程**（VMM、I/O、Kata Shim）移到一个独立的 `/kata_overhead/<sandbox_id>` cgroup 中，该 cgroup **不受任何资源限制**：

```
/kubepods/pod-xxx/                     /kata_overhead/
  └── kata_<sandbox_id>/                 └── <sandbox_id>/
        └── vCPU 线程（仅此）                  ├── QEMU/CLH 进程
                                               ├── I/O 线程
                                               └── Kata Shim
```

⚠️ **风险**: VMM 等进程在不受限的 overhead cgroup 中运行，可能消耗大量 host 资源；Pod 级 cgroup `memory.current` 严重低估 VMM 开销（这正是我们测试中 `kubectl top pod` 报极低值的原因——QEMU 的共享内存和文件映射不被 cgroup 计入）。

**生产建议**: 配置 Pod Overhead（见第 4 节）+ 设置 `sandbox_cgroup_only = true`，确保所有 Kata 进程在 Pod cgroup 内受限并可观测。
...
- ❌ `kubectl top pod`（读取 cgroup 实时用量，cgroup memory.current 严重低估 VM 开销，仍报极低值）
...
| **监控盲区** | `kubectl top pod` 严重低估 VM 开销（cgroup 的 MAP_SHARED 和文件映射不计入机制导致）；需配合 node-level 监控（如 node_memory_MemAvailable） |
...
| VM 开销对 K8s cgroup 几乎不可见 | 两种 VMM cgroup memory.current 严重低估（<1 MiB vs 实际 150-275 MiB） | ❌ 过度调度风险 |
...
**一句话建议**: 在生产环境使用 Kata Containers 时，**必须**在 RuntimeClass 中配置 `overhead.podFixed`，否则 Kubernetes 调度器会因 cgroup 严重低估的 VM 内存开销而过度调度，最终导致节点 OOM。kata-qemu 和 kata-clh 都存在相同问题。
